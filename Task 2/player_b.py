...
def player_b():
    rospy.init_node('player_b', anonymous=True)

    rospy.sleep(1)  # Wait for server to initialize

    pub = rospy.Publisher('player_b_moves', String, queue_size=1)
    rospy.Subscriber('moderator', String, callback)

    while not rospy.is_shutdown():
        fire_move = int(input("Fire's Turn (1 for Attack One, 2 for Attack All): "))
        if fire_move == 1:
            monsterattack = int(input("Enter the name of monster to attack (1 for Fire, 2 for Water and 3 for Earth): "))
